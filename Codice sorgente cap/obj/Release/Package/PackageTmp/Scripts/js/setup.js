...
;(function($, window, document, undefined)
{
	...
	$.template.init = function()
	{
		...
		$('#open-menu').on('touchend click', function(event)
		{
            ...
            bod.toggleClass(flagOpen ? 'menu-hidden' : 'menu-open');
            var flagOpen = "";
            flagOpen = bod.hasClass("menu-hidden").toString()
            if(flagOpen=="true")
                setSessionOpenCloseMenuDX('menu-hidden');
            else 
                setSessionOpenCloseMenuDX('menu-open');
   ...
   function setSessionOpenCloseMenuDX(val) 
        {
           // $.post('/SetSession/SetVariable', 
            $.post('/B16/SetSessionVariable', 
            { key : "OpenCloseMenuDX", value : val }, function (data)
            {/*alert("Success " + data.success + " " + val);*/});
        }